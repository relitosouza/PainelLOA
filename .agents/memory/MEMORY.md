...
- [relatorio-loa-colunas] Relatório LOA impresso/PDF atualizado (`src/lib/loa-report-template.ts`, `src/components/analise-loa-view.tsx`): remoção da coluna Valor LDO mantendo o card superior; renomeação de Valor LOA para Valor Solicitado; inclusão das colunas Ajuste SF e Corte GP; novo total calculado como `Valor LOA + Reajuste + Aditamento + Ajuste SF + Corte GP`.
