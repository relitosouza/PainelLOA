...
export async function GET(req: NextRequest) {
  try {
    ...
    let registros: any[] = await (db as any).ldoReceita.findMany({
    ...
    if (registros.length === 0 && exercicio) {
      ...
      registros = await (db as any).ldoReceita.findMany({
        orderBy: [{ exercicio: "desc" }, { vinculo: "asc" }],
      });
    }

    const totalPrevistoLdo = registros.reduce((acc: number, r: any) => acc + Number(r.valorTotalLdo || 0), 0);
    const totalDistribuidoLoa = registros.reduce((acc: number, r: any) => acc + Number(r.valorDistribuidoLoa || 0), 0);
    const saldoNaoDistribuido = totalPrevistoLdo - totalDistribuidoLoa;

    const vinculosSet = new Set(registros.map((r: any) => r.vinculo));
    const totalVinc = vinculosSet.size;

    const totalmenteDistribuido = registros.filter((r: any) => r.statusDistribuicao === "TOTALMENTE_DISTRIBUIDO").length;
    const parcialmenteDistribuido = registros.filter((r: any) => r.statusDistribuicao === "PARCIALMENTE_DISTRIBUIDO").length;
    const naoIniciado = registros.filter((r: any) => r.statusDistribuicao === "NAO_INICIADO").length;
    const acimaLdo = registros.filter((r: any) => r.statusDistribuicao === "ACIMA_LDO").length;
