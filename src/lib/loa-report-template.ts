...
export function generateLoaReportHtml(data: LoaReportData): string {
  ...
  const groupsHtml = data.groups.map((group) => {
    ...
    const itemsHtml = group.items.map((item) => {
      ...
      const iValLdo = formatTableCell(item.valLdo);
...
<tr class="zebra-row border-b border-outline-variant hover:bg-surface-container-low transition-colors">
  ...
  <td class="p-padding-cell-v px-padding-cell-h text-right">${iValLdo}</td>
...
<title>Relatório Técnico - ${secretaria}</title>
