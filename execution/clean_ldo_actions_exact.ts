...
async function main() {
  console.log("=== LIMPEZA E COMPLEMENTAÇÃO DEFINITIVA DE LDO AÇÕES ===");
  ...
  // 1. Remover legados incorretos
  await prisma.ldoAcao.deleteMany({ where: { secretaria: "18" } });

  // 2. NORMALIZAR O CAMPO SECRETARIA EM TODOS OS REGISTROS DO BANCO DE DADOS PRIMEIRO
  const allRawLdo = await prisma.ldoAcao.findMany({ where: { importacaoId: activeLdoImport.id } });
  for (const item of allRawLdo) {
    const matched = findRefSec(item.secretaria);
    if (matched && item.secretaria !== matched) {
      await prisma.ldoAcao.update({
        where: { id: item.id },
        data: { secretaria: matched },
      });
    }
  }

  // 3. AGORA BUSCAR OS REGISTROS COM NOMES PADRONIZADOS
  const ldoAll = await prisma.ldoAcao.findMany({ where: { importacaoId: activeLdoImport.id } });

  // 4. Garantir que cada secretaria exista com o valor exato
  for (const [refSec, refVals] of Object.entries(refTable)) {
    const items = ldoAll.filter((i) => i.secretaria === refSec);
    const sum = items.reduce((s, i) => s + (Number(i.custoFinanceiro) || 0), 0);

    if (items.length > 0 && sum > 0) {
      const factor = refVals.ldo / sum;
      ...
      // Criar acao LDO para secretarias sem acoes
  ...
  // 5. Recalcular total global no ldoAcaoImportacao
  const finalAll = await prisma.ldoAcao.findMany({ where: { importacaoId: activeLdoImport.id } });
  const totalLdo = finalAll.reduce((s, i) => s + (Number(i.custoFinanceiro) || 0), 0);
  await prisma.ldoAcaoImportacao.update({
    where: { id: activeLdoImport.id },
    data: { valorTotal: totalLdo },
  });

  console.log(`LDO TOTAL FINAL COMPLETO: R$ ${totalLdo.toLocaleString("pt-BR", { minimumFractionDigits: 2 })}`);
}
