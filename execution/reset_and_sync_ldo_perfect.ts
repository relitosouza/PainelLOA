...
async function main() {
  console.log("=== PERFECT MATCH LDO SCALING ===");
  ...
  for (const [refSec, refVals] of Object.entries(refTable)) {
    const items = ldoAll.filter((i) => matches(i.secretaria, refSec));
  ...
  // Recalcular total LDO
  const finalAllLdo = await prisma.ldoAcao.findMany({ where: { importacaoId: activeLdoImport.id } });
  const totalLdo = finalAllLdo.reduce((s, i) => s + (Number(i.custoFinanceiro) || 0), 0);
  await prisma.ldoAcaoImportacao.update({
    where: { id: activeLdoImport.id },
    data: { valorTotal: totalLdo },
  });

  console.log(`LDO TOTAL FINAL: R$ ${totalLdo.toLocaleString("pt-BR", { minimumFractionDigits: 2 })}`);
}
