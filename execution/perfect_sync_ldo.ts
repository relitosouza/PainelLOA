...
async function main() {
  console.log("=== EXECUÇÃO DA SINCRONIZAÇÃO PERFEITA DE LDO E LOA ===");
  ...
  for (const [refSec, refVals] of Object.entries(refTable)) {
    // 1. LDO
    const matchingLdo = allLdo.filter((i) => i.secretaria.trim() === refSec);
    ...
    // 2. LOA
    const matchingLoa = allLoa.filter((i) => i.organ.trim() === refSec);
