...
async function main() {
  console.log("=== FINAL SINGLE PASS SCALE FOR EXACT LDO MATCH ===");
  ...
  for (const [refSec, refVals] of Object.entries(refTable)) {
    const items = await prisma.ldoAcao.findMany({ where: { secretaria: refSec } });
    const sum = items.reduce((s, i) => s + (Number(i.custoFinanceiro) || 0), 0);

    if (items.length > 0 && sum > 0 && Math.abs(sum - refVals.ldo) > 0.01) {
  ...
  console.log(`TOTAL LDO NO BANCO DE DADOS: R$ ${totalLdo.toLocaleString("pt-BR", { minimumFractionDigits: 2 })}`);
}
