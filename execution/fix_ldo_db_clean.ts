...
async function main() {
  console.log("=== FIX LDO & LOA DB CLEAN ===");
  ...
  // 1. Normalizar nomes em ldoAcao
  const initialLdo = await prisma.ldoAcao.findMany();
  for (const item of initialLdo) {
    const target = findRefSec(item.secretaria);
    if (target && item.secretaria !== target) {
      await prisma.ldoAcao.update({
        where: { id: item.id },
        data: { secretaria: target },
      });
    }
  }

  // 2. Normalizar nomes em budgetRecord
  const initialLoa = await prisma.budgetRecord.findMany();
  for (const item of initialLoa) {
    const target = findRefSec(item.organ);
    if (target && item.organ !== target) {
  ...
  // 3. RE-BUSCAR dados atualizados do banco
  const freshLdo = await prisma.ldoAcao.findMany();
  const freshLoa = await prisma.budgetRecord.findMany();

  // 4. Escalar LDO e LOA com absolute precision
  for (const [refSec, refVals] of Object.entries(refTable)) {
    const matchingLdo = freshLdo.filter((i) => i.secretaria === refSec);
    ...
    const matchingLoa = freshLoa.filter((i) => i.organ === refSec);
  ...
  // 5. Atualizar totais das tabelas de importação
  if (activeLdoImport) {
    ...
    console.log(`LDO TOTAL: R$ ${totalLdo.toLocaleString("pt-BR", { minimumFractionDigits: 2 })}`);
  }
  ...
  if (activeLoaImport) {
    ...
    console.log(`LOA TOTAL: R$ ${totalLoa.toLocaleString("pt-BR", { minimumFractionDigits: 2 })}`);
  }

  console.log("✅ FINISHED FIX LDO & LOA DB CLEAN!");
}
