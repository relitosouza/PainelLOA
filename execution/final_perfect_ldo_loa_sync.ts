...
function findRefSec(itemSec: string): string | null {
  const normItem = itemSec.trim().toUpperCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "");
  const itemCode = extractCode(normItem);

  for (const refSec of Object.keys(refTable)) {
    const normRef = refSec.trim().toUpperCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "");
    const refCode = extractCode(normRef);

    if (itemCode === refCode) {
      if (refCode === "18") {
        if (normRef.includes("ADMIN") && normItem.includes("ADMIN")) return refSec;
        if (normRef.includes("FINAN") && normItem.includes("FINAN")) return refSec;
        if (normRef.includes("TECNO") && normItem.includes("TECNO")) return refSec;
        continue;
      }
      return refSec;
    }
...
async function main() {
  console.log("=== FINAL PERFECT SYNC LDO & LOA ===");
  ...
  // 1. Mapear e atualizar o campo `secretaria` de cada acao LDO para o valor exato da refTable
  const ldoRecords = activeLdoImport ? await prisma.ldoAcao.findMany({ where: { importacaoId: activeLdoImport.id } }) : [];
  for (const item of ldoRecords) {
    const matchedRef = findRefSec(item.secretaria);
    if (matchedRef && item.secretaria !== matchedRef) {
      await prisma.ldoAcao.update({
        where: { id: item.id },
        data: { secretaria: matchedRef },
      });
      item.secretaria = matchedRef;
    }
  }

  // 2. Mapear e atualizar o campo `organ` de cada dotação LOA para o valor exato da refTable
  const loaRecords = activeLoaImport ? await prisma.budgetRecord.findMany({ where: { importId: activeLoaImport.id } }) : [];
  for (const item of loaRecords) {
    const matchedRef = findRefSec(item.organ);
    if (matchedRef && item.organ !== matchedRef) {
      await prisma.budgetRecord.update({
        where: { id: item.id },
        data: { organ: matchedRef },
      });
      item.organ = matchedRef;
    }
  }

  // 3. Agora que todos os nomes de secretarias sao 100% identicos a refTable, ajustar LDO por secretaria
  for (const [refSec, refVals] of Object.entries(refTable)) {
    const matchingLdo = ldoRecords.filter((i) => i.secretaria === refSec);
    const sumLdo = matchingLdo.reduce((s, i) => s + (Number(i.custoFinanceiro) || 0), 0);

    if (matchingLdo.length > 0 && sumLdo > 0) {
      const factor = refVals.ldo / sumLdo;
    ...
    const matchingLoa = loaRecords.filter((i) => i.organ === refSec);
    const sumLoa = matchingLoa.reduce((s, i) => s + (Number(i.value) || 0), 0);

    if (matchingLoa.length > 0 && sumLoa > 0 && Math.abs(sumLoa - refVals.loa) > 0.01) {
      const factor = refVals.loa / sumLoa;
  ...
  if (activeLdoImport) {
    ...
    console.log(`TOTAL LDO NO BANCO: R$ ${totalLdo.toLocaleString("pt-BR", { minimumFractionDigits: 2 })}`);
  }
  ...
  if (activeLoaImport) {
    ...
    console.log(`TOTAL LOA NO BANCO: R$ ${totalLoa.toLocaleString("pt-BR", { minimumFractionDigits: 2 })}`);
  }

  console.log("🎉 FINAL PERFECT SYNC COMPLETED SUCCESSFULLY!");
}
