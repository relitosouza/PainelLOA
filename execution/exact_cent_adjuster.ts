...
async function main() {
  console.log("=== AJUSTE EXATO DE CENTAVOS DE ARREDONDAMENTO ===");
  ...
  for (const [refSec, refVals] of Object.entries(refTable)) {
    // LDO
    const ldoItems = await prisma.ldoAcao.findMany({ where: { secretaria: refSec }, orderBy: { id: "asc" } });
    if (ldoItems.length > 0) {
      const sum = ldoItems.reduce((s, i) => s + Number(i.custoFinanceiro), 0);
      const diff = Number((refVals.ldo - sum).toFixed(2));
      if (diff !== 0) {
        const last = ldoItems[ldoItems.length - 1];
        const newVal = Number((Number(last.custoFinanceiro) + diff).toFixed(2));
        await prisma.ldoAcao.update({
          where: { id: last.id },
          data: { custoFinanceiro: newVal },
        });
      }
    }

    // LOA
    const loaItems = await prisma.budgetRecord.findMany({ where: { organ: refSec }, orderBy: { id: "asc" } });
    if (loaItems.length > 0) {
      const sum = loaItems.reduce((s, i) => s + Number(i.value), 0);
      const diff = Number((refVals.loa - sum).toFixed(2));
      if (diff !== 0) {
        const last = loaItems[loaItems.length - 1];
        const newVal = Number((Number(last.value) + diff).toFixed(2));
  ...
  // Recalcular totais exatos nas tabelas de importação
  if (activeLdoImport) {
    const ldoAll = await prisma.ldoAcao.findMany({ where: { importacaoId: activeLdoImport.id } });
    const totalLdo = ldoAll.reduce((s, i) => s + Number(i.custoFinanceiro), 0);
    await prisma.ldoAcaoImportacao.update({
      where: { id: activeLdoImport.id },
      data: { valorTotal: totalLdo },
    });
    console.log(`LDO TOTAL FINAL PERFEITO: R$ ${totalLdo.toLocaleString("pt-BR", { minimumFractionDigits: 2 })}`);
  }

  if (activeLoaImport) {
    const loaAll = await prisma.budgetRecord.findMany({ where: { importId: activeLoaImport.id } });
    const totalLoa = loaAll.reduce((s, i) => s + Number(i.value), 0);
    await prisma.loaImport.update({
      where: { id: activeLoaImport.id },
      data: { totalValue: totalLoa },
    });
    console.log(`LOA TOTAL FINAL PERFEITO: R$ ${totalLoa.toLocaleString("pt-BR", { minimumFractionDigits: 2 })}`);
  }

  console.log("🎉 AJUSTE CENTAVO A CENTAVO CONCLUÍDO COM SUCESSO!");
}
